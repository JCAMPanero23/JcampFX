...
class AccountState:
    ...
    # Phase 3.7: SwingRider gates
    any_partial_hit: bool = False    # Track if any trade hit partial exit (for Gate 9.6)
    peak_equity: float = 500.0       # Track peak for DD calculation (for Gate 9.5)
    ...
    def get_current_drawdown_pct(self) -> float:
        """Calculate current DD from peak equity (for SwingRider Gate 9.5)."""
...
class BrainCore:
    ...
    def process(
        ...
        ohlc_daily: Optional[pd.DataFrame] = None,  # Phase 3.7: SwingRider daily regime filter
    ) -> Optional[Signal]:
        """
        ...
        # Actual enforcement happens at Gate 2.5 after strategy selection
        ...
        # Note: Gate 2.5 enforces 5-slot limit for TrendRider/RangeRider
        # SwingRider gets dedicated 6th slot
        ...
        # Phase 3.7: Pass pair for pair-aware strategy selection (SwingRider only takes GBPJPY)
        active_strategy = self._select_strategy(composite_score, pair)
        if active_strategy is None:
            return None

        # --- Gate 7.4: GBPJPY exclusivity for SwingRider (Phase 3.7) ---
        # Block TrendRider/RangeRider from trading GBPJPY if SwingRider is registered
        if pair == "GBPJPY" and active_strategy.name != "SwingRider":
            # Check if SwingRider is registered
            has_swingrider = any(s.name == "SwingRider" for s in self._strategies)
            if has_swingrider:
                reason = f"GBPJPY_EXCLUSIVE:SwingRider blocks {active_strategy.name} on GBPJPY"
                log.info("%s: %s", pair, reason)
                return _blocked_signal(pair, composite_score, current_time, reason)
        ...
        # ===================================================================
        # Phase 3.7: SwingRider-Specific Gates
        # ===================================================================

        # --- Gate 2.5: SwingRider dedicated 6th slot (Phase 3.7: 5+1 model) ---
        # TrendRider/RangeRider: Max 5 concurrent (standard limit, EXCLUDES SwingRider trades)
        # SwingRider: Gets dedicated 6th slot (max 1 SwingRider, independent of other strategies)
        from src.config import SWINGRIDER_MAX_CONCURRENT
        swingrider_trades = [
            t for t in account_state.open_positions if t.get("strategy") == "SwingRider"
        ]
        open_count = account_state.open_position_count()
        # BUG FIX (Phase 3.7): Non-SwingRider count excludes SwingRider's dedicated slot
        ...
        if active_strategy.name == "SwingRider":
            ...
            if len(swingrider_trades) >= SWINGRIDER_MAX_CONCURRENT:
                ...
                log.info("%s: SwingRider max concurrent reached (%d)", pair, len(swingrider_trades))
                return signal
            # SwingRider allowed even if at 5 other positions (gets 6th slot)
        else:
            # TrendRider/RangeRider: Max 5 concurrent (EXCLUDING SwingRider trades)
            # BUG FIX: SwingRider's dedicated slot doesn't count against the 5-slot limit
            if non_swingrider_count >= 5:
                signal.blocked_reason = "MAX_CONCURRENT_5"
                log.debug("%s: Max 5 concurrent for TrendRider/RangeRider (non-SR: %d)", pair, non_swingrider_count)
                return signal

        # --- Gate 9.5: Portfolio DD gate (SwingRider only) ---
        if active_strategy.name == "SwingRider":
            from src.config import SWINGRIDER_PORTFOLIO_DD_GATE
            dd_pct = account_state.get_current_drawdown_pct()
            if dd_pct > SWINGRIDER_PORTFOLIO_DD_GATE:
                signal.blocked_reason = f"PORTFOLIO_DD_GATE:{dd_pct:.1f}%>{SWINGRIDER_PORTFOLIO_DD_GATE}%"
                log.info("%s: %s", pair, signal.blocked_reason)
                return signal

        # --- Gate 9.6: Momentum confirmation (SwingRider only) ---
        # Phase 3.7: DISABLED for original convex design implementation
        # Original design does not require momentum confirmation
        # if active_strategy.name == "SwingRider":
        #     if not account_state.any_partial_hit:
        #         signal.blocked_reason = "NO_PARTIAL_MOMENTUM"
        #         log.info("%s: SwingRider requires at least one partial hit first", pair)
        #         return signal
        ...
        risk_result = compute_trade_risk(
            ...
            strategy_base_risk=getattr(active_strategy, "base_risk_pct", None),  # Phase 3.7
        )
        ...
        signal.strategy_obj = active_strategy  # Phase 3.7: Attach strategy object for exit config
    ...
    def _select_strategy(self, composite_score: float, pair: str = "") -> Optional[BaseStrategy]:
        """
        Return the first registered strategy whose regime range contains composite_score
        AND allows the given pair.

        Phase 3.7: Pair-aware strategy selection.
        - If strategy.allowed_pairs is None, the strategy accepts all pairs.
        - If strategy.allowed_pairs is a list, the strategy only accepts those pairs.
        - This allows SwingRider (GBPJPY-only, CS 90-100) to coexist with TrendRider
          (all pairs, CS 70-100) without SwingRider stealing signals from other pairs.
        """
        for strategy in self._strategies:
            if strategy.is_regime_active(composite_score):
                # Check pair restriction (Phase 3.7)
                allowed = getattr(strategy, "allowed_pairs", None)
                if allowed is not None and pair not in allowed:
                    continue  # Skip this strategy, try next matching one
                return strategy
        return None
