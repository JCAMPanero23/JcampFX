...
class BacktestEngine:
    ...
    def __init__(
        ...
        self._ohlc_daily: dict[str, pd.DataFrame] = {}  # Phase 3.7: For SwingRider
        self._dcrd_available = True
        for pair in pairs:
            h4 = self._load_ohlc(pair, "4h")
            h1 = self._load_ohlc(pair, "1h")
            m15 = self._load_ohlc(pair, "m15")
            daily = self._load_ohlc(pair, "daily")  # Phase 3.7: SwingRider daily OHLC
    ...
    def run(
        ...
        while heap:
            ...
            # 2. Daily 2R cap — if hit, force-close remaining positions (only close trades <-0.5R)
            if account.is_daily_cap_hit():
                ...
                for trade in list(account.open_trades):
                    ...
                    r_distance = abs(trade.entry_price - trade.sl_price)
                    if trade.direction.upper() == "BUY":
                        current_r = (close_px - trade.entry_price) / r_distance if r_distance > 0 else 0.0
                    else:  # SELL
                        current_r = (trade.entry_price - close_px) / r_distance if r_distance > 0 else 0.0

                    # Only close trades losing more than 0.5R (protect near-breakeven and winners)
                    if current_r >= -0.5:
                        continue  # Skip closing - trade is near breakeven or profitable
            ...
            if _is_near_friday_close(end_time):
                ...
                for trade in list(account.open_trades):
                    ...
                    # Only close trades losing more than 0.5R (protect near-breakeven and winners)
                    if current_r >= -0.5:
                        continue  # Skip closing - let near-breakeven and profitable trades run over weekend
            ...
            ohlc_daily = self._get_ohlc_window(pair, "daily", end_time)  # Phase 3.7: SwingRider
            ...
            signal = self._brain.process(
                ...
                ohlc_daily=ohlc_daily,  # Phase 3.7: SwingRider daily regime filter
    ...
    def _check_exits_on_bar(
        ...
        if trade.phase == "open":
            ...
            # Check partial exit (Phase 3.7: Uses strategy-specific R target)
            from src.exit_manager import calculate_partial_exit_price
            partial_r = getattr(trade.strategy_obj, "partial_exit_r", 1.5) if trade.strategy_obj else 1.5
            target_partial = calculate_partial_exit_price(trade.entry_price, trade.sl_price, trade.direction, partial_r)
            at_partial = (
                bar_high >= target_partial
                if trade.direction.upper() == "BUY"
                else bar_low <= target_partial
            )
            if at_partial:
                fill_partial = tick_boundary if (is_phantom_bar or is_gap_adj) else target_partial
                account.apply_partial_exit(trade, fill_partial, timestamp, atr14)

        elif trade.phase == "runner":
            # Phase 3.7: SwingRider uses daily Chandelier (NOT Range Bar trailing)
            if trade.strategy == "SwingRider":
                # SwingRider runner exit logic
                self._check_swingrider_runner_exits(
                    trade, bar, account, timestamp, current_composite_score,
                    is_phantom_bar, is_gap_adj, tick_boundary, bar_close, bar_high, bar_low
                )
            else:
                # All other strategies use Range Bar trailing (original logic)
                ...
                if not bar_is_trend_direction:
                    ...
                    # 2-bar auto-close rule (DISABLED FOR TEST)
    ...
    def _check_swingrider_runner_exits(
        self,
        trade: BacktestTrade,
        bar: pd.Series,
        account: BacktestAccount,
        ...
        """
        SwingRider runner exit logic (Phase 3.7 — daily Chandelier).

        Exit triggers:
        1. Hard invalidation (opposite structure or EMA50 penetration)
        2. Daily Chandelier SL hit

        Updates Chandelier SL once per daily close (NOT every bar).
        """
        from src.config import (
            SWINGRIDER_HARD_INVALIDATION_ENABLED,
            SWINGRIDER_CHANDELIER_ATR_PERIOD,
            SWINGRIDER_CHANDELIER_MULTIPLIER_NORMAL,
            SWINGRIDER_CHANDELIER_MULTIPLIER_EXPANSION,
        )
        ...
        # Get daily OHLC for this pair
        ohlc_daily = self._get_ohlc_window(trade.pair, "daily", timestamp, n=100)
        if ohlc_daily is None or len(ohlc_daily) < 50:
            log.warning(f"SwingRider {trade.trade_id}: No daily OHLC available, using fallback Range Bar exit")
            # Fallback to Range Bar trailing if daily OHLC unavailable
            return

        # 1. Check hard invalidation (rare structural exit)
        if SWINGRIDER_HARD_INVALIDATION_ENABLED:
            if check_hard_invalidation(trade.direction, ohlc_daily):
                fill_price = tick_boundary if (is_phantom_bar or is_gap_adj) else bar_close
                account.close_trade(trade, fill_price, timestamp, "HARD_INVALIDATION")
                return

        # 2. Update daily Chandelier SL (once per day)
        current_date = timestamp.date()

        # Check if this is a new daily candle (update Chandelier)
        if trade.last_chandelier_update_date is None or current_date > trade.last_chandelier_update_date:
            # Calculate ATR22 on daily chart
            atr22 = calculate_atr(ohlc_daily, period=SWINGRIDER_CHANDELIER_ATR_PERIOD)

            # Check for volatility expansion
            vol_expansion = is_volatility_expansion(ohlc_daily)

            # Select multiplier (3.0 normal, 2.2 during expansion)
            if vol_expansion:
                multiplier = SWINGRIDER_CHANDELIER_MULTIPLIER_EXPANSION
                if not trade.volatility_accelerator_active:
                    log.info(f"SwingRider {trade.trade_id}: Volatility accelerator activated (2.2× multiplier)")
                    trade.volatility_accelerator_active = True
            else:
                multiplier = SWINGRIDER_CHANDELIER_MULTIPLIER_NORMAL
                if trade.volatility_accelerator_active:
                    log.info(f"SwingRider {trade.trade_id}: Volatility accelerator deactivated (3.0× multiplier)")
                    trade.volatility_accelerator_active = False

            # Calculate Chandelier distance
            chandelier_distance = atr22 * multiplier

            # Find highest high (BUY) or lowest low (SELL) since entry
            trade_daily_bars = ohlc_daily[ohlc_daily['time'] >= trade.entry_time]

            if len(trade_daily_bars) > 0:
                if trade.direction.upper() == "BUY":
                    highest_high = trade_daily_bars['high'].max()
                    proposed_sl = highest_high - chandelier_distance

                    # Never move SL backwards
                    new_sl = max(trade.trailing_sl, proposed_sl)

                else:  # SELL
                    lowest_low = trade_daily_bars['low'].min()
                    proposed_sl = lowest_low + chandelier_distance

                    # Never move SL backwards
                    new_sl = min(trade.trailing_sl, proposed_sl)

                # Update trailing SL
                if new_sl != trade.trailing_sl:
                    log.debug(
                        f"SwingRider {trade.trade_id}: Chandelier updated {trade.trailing_sl:.3f} → {new_sl:.3f} "
                        f"(ATR22={atr22:.3f}, mult={multiplier:.1f})"
                    )
                    trade.trailing_sl = new_sl

                # Update last update date
                trade.last_chandelier_update_date = current_date

        # 3. Check trailing SL hit
        sl_hit = (
            bar_low <= trade.trailing_sl
            if trade.direction.upper() == "BUY"
            else bar_high >= trade.trailing_sl
        ...
        if sl_hit:
            fill_price = tick_boundary if (is_phantom_bar or is_gap_adj) else trade.trailing_sl
            account.close_trade(trade, fill_price, timestamp, "CHANDELIER_DAILY_HIT")
    ...
    def _open_trade(
        ...
        trade = BacktestTrade(
            ...
            strategy_obj=signal.strategy_obj,  # Phase 3.7: Strategy object reference
            ...
            swing_level=signal.swing_level or 0.0,  # Phase 3.7: SwingRider swing level
        )
    ...
    def _load_ohlc(self, pair: str, timeframe: str) -> Optional[pd.DataFrame]:
        """Load OHLC data (4H, 1H, M15, or Daily). Returns None if file not found."""
        if timeframe == "4h":
            ...
            # Phase 3.7: SwingRider daily OHLC (resampled from 4H if not cached)
            from data_loader.daily_ohlc import load_daily_ohlc_for_pair
            try:
                df = load_daily_ohlc_for_pair(pair)
                return df.reset_index()  # Return with 'time' column (not as index)
            except Exception as exc:
                log.warning("Failed to load daily OHLC for %s: %s", pair, exc)
                return None
        else:
            return None
