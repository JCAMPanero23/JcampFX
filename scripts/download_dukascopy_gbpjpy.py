"""
Download GBPJPY historical tick data from Dukascopy (2018-2024).

Dukascopy provides free historical tick data for forex pairs.
We'll download and convert to JcampFX tick format.
"""
...
def download_hour_ticks(pair: str, year: int, month: int, day: int, hour: int) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            log.warning(f"LZMA decompress failed for {year}-{month:02d}-{day:02d} {hour:02d}h")
            return []
        ...
        if "404" not in str(e):
            log.debug(f"Error downloading {year}-{month:02d}-{day:02d} {hour:02d}h: {e}")
        return []
...
def download_month_ticks(pair: str, year: int, month: int) -> pd.DataFrame:
    ...
    log.info(f"Downloading {pair} {year}-{month:02d} ({days_in_month} days)...")
    ...
    for day in range(1, days_in_month + 1):
        ...
        if not day_df.empty:
            ...
            log.info(f"  {year}-{month:02d}-{day:02d}: {len(day_df):,} ticks")

    if not month_ticks:
        log.warning(f"No ticks for {year}-{month:02d}")
        return pd.DataFrame()

    df = pd.concat(month_ticks, ignore_index=True)
    df = df.sort_values('time').reset_index(drop=True)

    log.info(f"  Total for {year}-{month:02d}: {len(df):,} ticks")

    return df


def download_year_ticks(pair: str, year: int) -> pd.DataFrame:
    """Download all 12 months for a year."""
    log.info(f"=== Downloading {pair} {year} ===")
    ...
    if not year_ticks:
        log.warning(f"No ticks for {year}")
        return pd.DataFrame()

    df = pd.concat(year_ticks, ignore_index=True)
    df = df.sort_values('time').reset_index(drop=True)

    log.info(f"=== Total for {year}: {len(df):,} ticks ===\n")
...
def save_ticks_parquet(df: pd.DataFrame, output_path: Path):
    ...
    log.info(f"Saved {len(tick_df):,} ticks to {output_path}")


def main():
    """Download GBPJPY 2018-2024 (skip 2024 July onwards as we have it from MT5)."""
    ...
    for year, start_month, end_month in years_to_download:
        log.info(f"\n{'=' * 60}")
        log.info(f"Downloading {PAIR_CODE} {year} (months {start_month}-{end_month})")
        log.info(f"{'=' * 60}\n")

        year_ticks = []

        for month in range(start_month, end_month + 1):
            month_df = download_month_ticks(PAIR_CODE, year, month)
            if not month_df.empty:
                year_ticks.append(month_df)
        ...
        if year_ticks:
            ...
            year_output = output_dir / f"GBPJPY_{year}_ticks.parquet"
    ...
    if all_ticks:
        log.info("\n" + "=" * 60)
        log.info("Combining all years...")
        log.info("=" * 60)

        combined_df = pd.concat(all_ticks, ignore_index=True)
        combined_df = combined_df.sort_values('time').reset_index(drop=True)

        combined_output = output_dir / "GBPJPY_2018-2024_ticks.parquet"
