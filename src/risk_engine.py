...
def calculate_risk_pct(
    ...
    strategy_base_risk: float | None = None,
) -> float:
    """
    Compute the final risk percentage.

    Phase 3.7: Supports strategy_base_risk override (SwingRider uses 0.7%).
    ...
    """
    # Use strategy override if provided, else BASE_RISK_PCT
    base_risk = strategy_base_risk if strategy_base_risk is not None else BASE_RISK_PCT
...
def compute_trade_risk(
    ...
    """
    confidence_mult = get_confidence_multiplier(composite_score)
    risk_pct = calculate_risk_pct(composite_score, performance_multiplier, strategy_base_risk)
