...
def get_partial_exit_pct(
    composite_score_at_entry: float,
    strategy_override: float | None = None
) -> float:
    """
    Return the fraction of the position to close at partial exit target.

    Phase 3.7: Supports strategy override (SwingRider sets 0.40 fixed).
    ...
    """
    # Phase 3.7: Strategy can override CS-based tiers
...
def calculate_partial_exit_price(
    entry: float,
    sl: float,
    direction: str,
    partial_r: float = 1.5
    ...
    """
    Calculate the price at which Stage 1 partial exit fires.

    Phase 3.7: Supports custom R-multiple target (SwingRider uses 2.0R).

    VE.1: partial exit fires at exactly partial_r * R from entry.
    """
    r_distance = abs(entry - sl)
    target = partial_r * r_distance
...
def calculate_1_5r_price(entry: float, sl: float, direction: str) -> float:
    """Legacy alias for calculate_partial_exit_price() at 1.5R."""
    return calculate_partial_exit_price(entry, sl, direction, partial_r=1.5)


def is_at_partial_target(
    current_price: float,
    entry: float,
    sl: float,
    direction: str,
    ...
    """
    Return True when price has reached or exceeded the partial exit target.

    Phase 3.7: Supports custom R-multiple (SwingRider uses 2.0R).
    ...
    """
    target = calculate_partial_exit_price(entry, sl, direction, partial_r)
    _eps = 1e-9
    if direction.upper() == "BUY":
...
def is_at_1_5r(current_price: float, entry: float, sl: float, direction: str) -> bool:
    """Legacy alias for is_at_partial_target() at 1.5R."""
    return is_at_partial_target(current_price, entry, sl, direction, partial_r=1.5)
...
def initial_chandelier_sl(
    ...
    Phase 3.7: Supports custom partial_r and chandelier_multiplier.

    chandelier_distance = max(0.5R, ATR14) * multiplier  in price terms (not pips)
    ...
    partial_r            : R-multiple for partial exit (default 1.5, SwingRider uses 2.0)
    chandelier_multiplier: Buffer multiplier (default 1.0, SwingRider uses 1.5)
    """
    ...
    chandelier_distance *= chandelier_multiplier  # Apply multiplier

    # Calculate partial exit price using custom R-multiple
    partial_exit_price = calculate_partial_exit_price(entry, sl, direction, partial_r)
...
def update_chandelier(
    ...
    Phase 3.7: Supports chandelier_multiplier for wider buffers (SwingRider uses 1.5).
