...
class BacktestTrade:
    ...
    strategy: str           # "TrendRider" | "BreakoutRider" | "RangeRider" | "SwingRider"
    ...
    # Phase 3.7: Strategy object reference for exit config (must come after required fields)
    ...
    # SwingRider daily Chandelier tracking (Phase 3.7 — original convex design)
    last_chandelier_update_date: Optional[date] = None  # Track daily updates (not every bar)
    volatility_accelerator_active: bool = False         # True when multiplier = 2.2 (vs 3.0)
    swing_level: float = 0.0                            # Pullback swing low/high for SL placement
