...
_INSPECTOR_VERSION = "2026-02-28-v18"
...
def _build_metadata_panel(ctx: dict) -> html.Div:
    ...
    # Calculate pip size for this pair
    pip_size = 0.01 if "JPY" in pair else 0.0001

    # Calculate SL distance in pips
    entry_price = trade['entry_price']
    sl_price = trade['sl_price']
    sl_distance_pips = abs(sl_price - entry_price) / pip_size
    ...
    duration_str = f"{int(duration.days)}d {int(duration_hours % 24)}h" if duration.days > 0 else f"{duration_hours:.1f}h"

    # Calculate move in pips (entry → close)
    close_price = trade.get('close_price', entry_price)
    direction = trade.get('direction', '')
    if direction == 'BUY':
        move_pips = (close_price - entry_price) / pip_size
    else:  # SELL
        move_pips = (entry_price - close_price) / pip_size
    ...
    metadata_rows = [
        ...
        html.Div([
            ...
            html.Span(f"Close: {close_price} @ {trade.get('close_time', 'N/A')}", style={"margin-right": "20px", "color": "#aaa"}),
        ]),
        html.Div([
            html.Span(f"SL: {trade['sl_price']} ({sl_distance_pips:.0f} pips)", style={"margin-right": "20px", "color": "#ff6b6b"}),
            html.Span(f"Move: {move_pips:+.1f} pips", style={"margin-right": "20px", "color": "#4fc3f7"}),
            html.Span(f"Duration: {duration_str}", style={"margin-right": "20px", "color": "#ffd54f"}),
    ...
    # Add partial exit details if hit
    partial_price = trade.get('partial_exit_price')
    if partial_price is not None and not pd.isna(partial_price):
        partial_time = pd.to_datetime(trade.get('partial_exit_time'))
        time_to_partial = partial_time - entry_time
        time_to_partial_hours = time_to_partial.total_seconds() / 3600
        time_to_partial_str = f"{int(time_to_partial.days)}d {int(time_to_partial_hours % 24)}h" if time_to_partial.days > 0 else f"{time_to_partial_hours:.1f}h"

        partial_pct = trade.get('partial_exit_pct', 0) * 100
        partial_r = trade.get('r_multiple_partial', 0)
        runner_r = trade.get('r_multiple_runner', 0)

        partial_meta = html.Div([
            html.Hr(style={"margin": "10px 0", "border-color": "#444"}),
            html.Div([
                html.Span(f"Partial Exit @ {partial_price} ({time_to_partial_str} after entry)",
                          style={"margin-right": "15px", "color": "#f0a500", "font-weight": "bold"}),
            ]),
            html.Div([
                html.Span(f"Closed {partial_pct:.0f}% @ {partial_r:.2f}R",
                          style={"margin-right": "15px", "color": "#888"}),
                html.Span(f"Runner (60%) @ {runner_r:.2f}R",
                          style={"margin-right": "15px", "color": "#888"}),
                html.Span(f"Close Reason: {trade.get('close_reason', 'N/A')}",
                          style={"margin-right": "15px", "color": "#888"}),
            ], style={"margin-top": "5px"}),
        ])
        metadata_rows.append(partial_meta)
