...
class BacktestAccount:
    ...
    peak_equity: float = field(init=False)  # Track peak for DD calculation (for Gate 9.5)
    ...
    def get_account_state(self) -> AccountState:
        ...
        positions = [
            {"pair": t.pair, "direction": t.direction, "strategy": t.strategy}
        ...
        return AccountState(
            ...
            any_partial_hit=self.any_partial_hit,
            peak_equity=self.peak_equity,
        )

    def mark_partial_exit(self) -> None:
        """Called when any trade hits partial exit (for SwingRider Gate 9.6)."""
        self.any_partial_hit = True

    def update_peak_equity(self) -> None:
        """Track peak equity for DD calculation (for SwingRider Gate 9.5)."""
    ...
    def apply_partial_exit(
        ...
        # Phase 3.7: SwingRider uses BE+0.2R, others use partial exit price - 5 pips
        if trade.strategy == "SwingRider":
            # SwingRider: Move SL to BE + 0.2R (protects runner)
            from src.config import SWINGRIDER_PARTIAL_SL_OFFSET_R
            r_distance = abs(trade.entry_price - trade.sl_price)

            if trade.direction.upper() == "BUY":
                trade.trailing_sl = trade.entry_price + (SWINGRIDER_PARTIAL_SL_OFFSET_R * r_distance)
            else:  # SELL
                trade.trailing_sl = trade.entry_price - (SWINGRIDER_PARTIAL_SL_OFFSET_R * r_distance)

            log.info(f"SwingRider {trade.trade_id}: SL moved to BE+0.2R @ {trade.trailing_sl:.3f}")
        else:
            # All other strategies: Range Bar trailing (Phase 3.1.1 — 3-bar SL system)
        ...
        # Phase 3.7: Mark that a partial exit occurred (for SwingRider Gate 9.6)
        self.mark_partial_exit()
